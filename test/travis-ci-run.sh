...
docker exec "${TEST_THUNDER_PHP_DOCKER_ID}" su - thunder --command='cd www; drush pml --status=enabled --format=json;' | jq --exit-status '.thunder_performance_measurement, .testsite_builder, .media_entity_generic'
