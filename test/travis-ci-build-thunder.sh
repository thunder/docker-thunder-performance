...
THUNDER_LATEST_ARTIFACT=$(curl -u "$GITHUB_ACCESS" 'https://api.github.com/repos/thunder/thunder-distribution/actions/workflows/test.yml/runs?branch=search_api-thunder35&conclusion=completed' --silent | jq --raw-output '.workflow_runs[0].artifacts_url')
