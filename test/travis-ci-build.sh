...
THUNDER_LATEST_BUILD_ID=$(curl --request GET 'https://api.travis-ci.com/v3/repo/thunder%2Fthunder-distribution/builds?branch.name=8.x-4.x&build.state=passed&build.event_type=cron,push&limit=1' --header 'Content-Type: application/json' --silent | jq --raw-output '.builds[0].id')
