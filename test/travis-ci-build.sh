...
THUNDER_LATEST_BUILD_ID=$(curl -X GET 'https://api.travis-ci.com/v3/repo/thunder%2Fthunder-distribution/builds?branch.name=8.x-3.x&build.state=passed&build.event_type=cron,push&limit=1' -H 'Content-Type: application/json' -s | jq --raw-output '.builds[0].id')
